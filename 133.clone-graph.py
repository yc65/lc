...
class Solution:
    def cloneGraph(self, node: 'Node') -> 'Node':
        ...
        queue = [node]
        # map the original node to the cloned 
        visited = {node:cloned_node}
        while queue:
            curr_node = queue.pop(0)
            for neighbour in curr_node.neighbors:
                if neighbour not in visited:
                    cloned_neighbor = Node(neighbour.val, [])
                    visited[curr_node].neighbors.append(cloned_neighbor)
                    visited[neighbour]= cloned_neighbor
                    queue.append(neighbour)
                else:
                    visited[curr_node].neighbors.append(visited[neighbour])
